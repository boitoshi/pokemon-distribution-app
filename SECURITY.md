...
- `src/pages/index.astro`:
  - `escapeHtml()` 関数の実装
  - ポケモン名のエスケープ
  - ボール名の表示を `textContent` に変更
  - 比較モーダルのすべてのフィールドをエスケープ
...
#### 修正した箇所
- `src/pages/index.astro`:
  - `getFavorites()` に try-catch 追加
  - `getNotes()` に try-catch 追加
  - `loadData()` でデータ検証追加
...
- 正本は `pokemon-data/distributions/*.json`。`scripts/sync-from-pokemon-data.mjs` で pull only 取り込み（件数減少ガード付き）
- ユーザーが直接編集することはできない
